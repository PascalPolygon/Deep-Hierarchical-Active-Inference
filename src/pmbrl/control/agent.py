...
class HierarchicalAgent(object):
    ...
    def run_episode(self, buffer=None, action_noise=None, recorder=None):
        ...
        with torch.no_grad():
            state = self.env.reset()
            while not done:
                # Generate a new high-level goal every c steps
                ...
                # Low-level policy acts towards the current high-level goal
                action = self.low_level_planner(state, self.current_goal)
            ...
            if self.high_level_timer == 0:
                # Update high-level planner after context ends
