...
class HierarchicalTrainer(object):
    ...
    def __init__(
        ...
        Args:
            ...
            low_level_ensemble_model (nn.Module): The low-level ensemble model.
        ...
        self.low_level_ensemble = low_level_ensemble_model
        self.buffer = buffer
        ...
        self.low_level_params = list(low_level_ensemble_model.parameters())
    ...
    def train(self):
        ...
        for epoch in range(1, self.n_train_epochs + 1):
            ...
            self._train_low_level(epoch, low_level_e_losses, n_batches_low)
        ...
        return (
            ...
            self._get_avg_loss(low_level_e_losses, n_batches_low, epoch), # low-level ensemble loss
        )
    ...
    def _train_low_level(self, epoch, e_losses, n_batches):
        """
        Training loop for the low-level models.
        ...
        Args:
            ...
            n_batches (list): List to store the number of batches per epoch.
        """
        for (states, actions, deltas) in self.buffer.get_low_level_train_batches(self.batch_size):
            self.low_level_ensemble.train()

            # Zero the gradients for the low-level model
            self.low_level_optim.zero_grad()

            # Compute loss for the low-level ensemble model
            e_loss = self.low_level_ensemble.loss(states, actions, deltas)
        ...
        if self.logger is not None and epoch % 20 == 0:
            avg_e_loss = self._get_avg_loss(e_losses, n_batches, epoch)
            message = "> Low-Level Train epoch {} [ensemble {:.2f}]"
            self.logger.log(message.format(epoch, avg_e_loss))
